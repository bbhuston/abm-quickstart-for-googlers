...
prepare-user-cluster:  ##     Copy a user cluster manifest to the workstation
	...
	@echo '-----------------------------------------------------------------------------------------------------'
	@sleep 3s
	@gsutil cp abm-clusters/user-cluster-001.yaml gs://${PROJECT_ID}-config-bucket/user-cluster-001.yaml
	@gcloud compute ssh root@abm-ws --zone ${ZONE} ${CORP_SETTINGS} << EOF
	mkdir -p bmctl-workspace/user-cluster-001
	gsutil cp gs://${PROJECT_ID}-config-bucket/user-cluster-001.yaml bmctl-workspace/user-cluster-001/user-cluster-001.yaml
