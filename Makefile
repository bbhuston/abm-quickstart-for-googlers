...
upload-kubevirt-image: check-for-existing-image  ##    Upload a VM image to KubeVirt
	...
	#cat temp.txt
	UPLOAD_PROXY_IP=$(kubectl get svc cdi-uploadproxy -n cdi --no-headers=true --kubeconfig=/root/bmctl-workspace/${CLUSTER_NAME}/${CLUSTER_NAME}-kubeconfig | awk '{print $$4}') && echo $$UPLOAD_PROXY_IP
