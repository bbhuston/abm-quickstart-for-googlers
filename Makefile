...
create-abm-cluster:  ##       Create an ABM cluster
	...
	@echo '-----------------------------------------------------------------------------------------------------'
	@sleep 3s
	@gsutil cp abm-clusters/${CLUSTER_NAME}.yaml gs://${PROJECT_ID}-config-bucket/${CLUSTER_NAME}.yaml
...
google-identity-login:  ##    Enable Google Identity Login
	...
	@echo '-----------------------------------------------------------------------------------------------------'
	@sleep 3s
	@gsutil cp anthos-features/google-identity-login/google-identity-login-rbac.yaml gs://${PROJECT_ID}-config-bucket/google-identity-login-rbac.yaml
	@gcloud compute ssh root@abm-ws --zone ${ZONE} ${CORP_SETTINGS} << EOF
	@gsutil cp gs://${PROJECT_ID}-config-bucket/google-identity-login-rbac.yaml google-identity-login-rbac.yaml
...
google-load-balancer:  ##     Enable Google Global Load Balancer
	...
	@gcloud compute ssh root@abm-ws --zone ${ZONE} ${CORP_SETTINGS} << EOF
	gsutil cp -r gs://${PROJECT_ID}-config-bucket/google-load-balancer .
	sed -i 's/PROJECT_ID/${PROJECT_ID}/' google-load-balancer/gce.conf
	sed -i 's/VPC/${VPC}/' google-load-balancer/gce.conf
	sed -i 's/ZONE/${ZONE}/' google-load-balancer/gce.conf
	...
	kubectl -n kube-system create configmap gce-config --from-file=google-load-balancer/gce.conf --kubeconfig=/root/bmctl-workspace/${CLUSTER_NAME}/${CLUSTER_NAME}-kubeconfig
...
cloud-build-hybrid:  ##       Enable Cloud Build Hybrid
	...
	@gsutil cp anthos-features/cloud-build-hybrid/cloud-build-hybrid-rbac.yaml gs://${PROJECT_ID}-config-bucket/cloud-build-hybrid-rbac.yaml
	@gcloud compute ssh root@abm-ws --zone ${ZONE} ${CORP_SETTINGS} << EOF
	@gsutil cp gs://${PROJECT_ID}-config-bucket/cloud-build-hybrid-rbac.yaml cloud-build-hybrid-rbac.yaml
...
upload-kubevirt-image:  ##    Upload a VM image to KubeVirt
	...
	@echo '-----------------------------------------------------------------------------------------------------'
	@echo
	@echo 	Uploading a KubeVirt image...
	@echo
	@echo '-----------------------------------------------------------------------------------------------------'
	@sleep 3s
	@gcloud compute ssh root@abm-ws --zone ${ZONE} ${CORP_SETTINGS} << EOF
...
create-kubevirt-vm:  ##       Create a KubeVirt VM
	@echo '-----------------------------------------------------------------------------------------------------'
	@echo
	@echo 	Creating a KubeVirt VM...
	@echo
	@echo '-----------------------------------------------------------------------------------------------------'
	@sleep 3s
	@gsutil cp anthos-features/kubevirt/kubevirt-example-001.yaml gs://${PROJECT_ID}-config-bucket/kubevirt-example-001.yaml
	@gcloud compute ssh root@abm-ws --zone ${ZONE} ${CORP_SETTINGS} << EOF
	gsutil cp gs://${PROJECT_ID}-config-bucket/kubevirt-example-001.yaml kubevirt-example-001.yaml
